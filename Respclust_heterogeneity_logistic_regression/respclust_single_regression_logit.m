function [result] = respclust_single_regression_logit(x, y, sample_size, row_size, col_size, beta_real, gamma_real,...
...
numSamples = size(x, 3);
reshapedX = zeros(numSamples, row_size * col_size);
for i = 1:numSamples
    reshapedX(i, :) = reshape(x(:,:,i), 1, []);
end
[class, k_best] = respclust_k_means_sc(reshapedX, k_min, k_max, num_time, sample_size);
