function [Result_table, Result_table_summary] = RESPCLUST_logit(simulation_size, Data_x, y_real, sample_size, row_size, col_size, beta_real, gamma_real,...
                         eps_initial, iter_max_initial_in, k_min, k_max, num_time, cv_number, tuning_start, tuning_end, tuning_number, if_fix)
%-------------------------------------函数功能----------------------------------------%
% 根据响应变量对样本进行k_means聚类，聚类个数由轮廓系数来进行选择，对聚类后的模型进行
% 矩阵变量线性回归，比较sc值，均方误差等参数
%-------------------------------------使用方法----------------------------------------%
% [Result_table, Result_table_summary] = RESPCLUST_logit(simulation_size, Data_x, y_real,...
...
for i = 1:simulation_size
    y = y_real(:, i);
    x = Data_x(:,:,:,i);
    [result] = respclust_single_regression_logit(x, y, sample_size, row_size, col_size, beta_real, gamma_real,...
